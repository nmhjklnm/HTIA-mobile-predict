...
class Seq2Seq(nn.Module):
    def __init__(self):
        ...
        self.attn = Self_Attention(method="mult_self", input_vector_dim=config.hidden_size, h=1)
    ...
    def forward(self,numpairs,attr,label):
        ...
        # encoder_outputs = self.attn(encoder_outputs, numpairs)
    ...
    def evaluate(self, numpairs, attr,topk):
        ...
        encoder_outputs, encoder_hidden = self.encoder(numpairs,attr)
        # encoder_outputs = self.attn(encoder_outputs, numpairs)
        return self.decoder.evaluatoin_beamsearch_heapq(encoder_outputs,encoder_hidden,numpairs,topk=topk)
